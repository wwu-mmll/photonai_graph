...
class SparseToDglTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # create array with sparse matrices
        self.np_array = np.array([sparse_matrix, sparse_short, sparse_matrix])
    ...
    def test_array_wrong_dim(self):
        with self.assertRaises(Exception):
            sparse_to_dgl(self.control_matrix)

    def test_np_array(self):
        g = sparse_to_dgl(self.np_array)
        self.assertEqual(type(g[0]), dgl.DGLGraph)
    ...
    def test_nonsense_list(self):
        with self.assertRaises(Exception):
            sparse_to_dgl([self.edge_dict] * 10)

    def test_nonsense_array(self):
        with self.assertRaises(Exception):
            sparse_to_dgl(np.array([self.edge_dict] * 10))
