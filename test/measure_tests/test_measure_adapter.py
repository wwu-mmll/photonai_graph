...
class MeasureAdapterTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.random_nx = [nx.erdos_renyi_graph(10) for i in range(10)]
        self.random_igraph = [igraph.Graph.Erdos_Renyi() for i in range(10)]
    ...
    def test_np_to_networkx(self):
        ...
        self.assertEqual(type(g[0]), nx.graph.Graph)
        self.assertEqual(type(g), np.ndarray)

    def test_networkx_to_networkx(self):
        adapter = GraphMeasureAdapter(output='networkx')
        g = adapter.transform(self.random_nx)
        self.assertEqual(type(g[0]), type(self.random_nx[0]))

    def test_networkx_type_error(self):
        adapter = GraphMeasureAdapter(output='networkx')
