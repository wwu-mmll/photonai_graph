...
class IgraphMeasureTransform(BaseEstimator, TransformerMixin):
    ...
    def handle_outputs(results, measure_list):
        ...
        if isinstance(results, dict):
            for rskey, rsval in results.items():
                IgraphMeasureTransform.handle_outputs(rsval, measure_list)
            return measure_list
        ...
        if isinstance(results, tuple):
            for result in results:
                IgraphMeasureTransform.handle_outputs(result, measure_list)
            return measure_list
